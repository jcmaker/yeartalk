...
export default function UploadPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!result.ok) {
        const errorMsg = result.error?.message || result.error?.code || "업로드 실패";
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-background p-3 sm:p-4 overflow-hidden">
      <FireworksBackground
        ...
        population={fireworksPopulation}
      />
      <div className="relative z-10 w-full max-w-md rounded-lg border bg-card/95 backdrop-blur-sm p-4 sm:p-6 shadow-sm">
        <div className="text-center mb-4 sm:mb-6">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">
            카카오톡 내보내기 파일을 업로드하여<br className="sm:hidden" /> 재미있는 회고를 만들어보세요
          </p>
        </div>
          ...
          {state === "idle" && (
            <div className="space-y-4">
              ...
              >
                ...
                <span className="text-sm sm:text-base font-medium mb-1">클릭하여 업로드</span>
